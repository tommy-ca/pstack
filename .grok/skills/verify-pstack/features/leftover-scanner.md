...
- `grok inspect --json` `.agents[].name` must include `pstack:swarm-workers` after enable. Only `pstack:poteto-agent` and `pstack:comment-sicko` means the enabled plugin is the Claude marketplace copy. Doctor companion is `python3 scripts/check-plugin-agents.py`. It is not leftover scanner.
