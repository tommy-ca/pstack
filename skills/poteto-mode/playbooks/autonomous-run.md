...
2. Pick the wake mechanism from grok-build primitives (`HARNESS.md`). An event to watch (CI, a merge, a ref advancing) gets `monitor` (`command`, `description`, `timeout_ms?`). Recurring re-check uses `/loop`, which expands to `scheduler_create` (`interval`, `prompt`, `fire_immediately: true`). One-shot delay is background `sleep && cmd`, not the scheduler. `/loop` is a grok-build slash command, not a pstack skill.
