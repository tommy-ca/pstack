...
1. Capture a baseline trace on the matching surface (profiler, CLI, or running app).
...
3. Plan the fix from the trace. If it crosses a function boundary, `architect` first. Delegate implementation to a subagent using toml key `perf-issue` (`skills/setup-pstack/references/resolve-model.md`; no toml: send `grok-4.6`, inherit-parent/auto/missing key: omit; `subagent_type: "pstack:perf-issue"` per `skills/setup-pstack/references/resolve-effort.md`; do not send `reasoning_effort` on `task`); review the diff. Capture a post-fix trace.
