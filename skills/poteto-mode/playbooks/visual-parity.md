...
4. Verify each component against its baseline via image diff on the matching surface (running app, screenshot harness, or Grok browser tools). A nonzero diff is a fail; investigate the pixel delta, don't wave it through. Persist the predicate and arm `scheduler_create` (and `monitor` if an event wake exists) per component until the diff is zero.
