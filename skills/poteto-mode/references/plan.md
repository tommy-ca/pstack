...
State your read of scope and constraints in one paragraph. Use `ask_user_question` only for genuinely ambiguous intent (the **never-block-on-the-human** principle skill); give concrete options with each open question.
...
- Resolve `model` from `~/.grok/pstack-models.toml` per `skills/setup-pstack/references/resolve-model.md`. Absent file: send `grok-4.6` (omit if rejected). Missing key, `inherit-parent`, or `auto`: omit `model`. Do not send `reasoning_effort` on `task`.
...
- Browser / Electron / Web UIs: drive the running app (Playwright, CDP, or the project's own harness).
- CLIs and TUIs: run the binary and assert output.
