...
Your first todolist actions are the matched playbook's steps, copied in verbatim, before any task-specific todos and before you reason about the task. The failure mode is reading a playbook then writing a bespoke plan that drops its named steps (`architect`, the throughput checkpoint). A step you choose not to do stays in the list with a one-line `skip: <reason>`; skipping silently is not allowed. Match the task to a playbook below, open git-tracked `skills/poteto-mode/playbooks/<name>.md`, and copy its steps in verbatim. Do not open `~/.grok/skills/shipping` or installed-plugin `playbooks/`. A comma-separated list of existing playbooks (review, reflect, learn, ship) is sequential matching. It is not Multi-phase-plan. That playbook writes a plan file and stops.
