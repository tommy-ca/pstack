...
**Defaults for every `task` call.** `run_in_background: true` (that is also grok-build's schema default). File pointers, not inlined dumps. Resolve `model` from `~/.grok/pstack-models.toml` per [`../setup-pstack/references/resolve-model.md`](../setup-pstack/references/resolve-model.md). Absent file: send `grok-4.6` (omit if rejected). Missing key, `inherit-parent`, or `auto`: omit `model`. Isolation is `none` unless the child must not touch the writer's tree, then `worktree`. Join with `get_task_output` (`task_ids`, positive `timeout_ms`). Do not send `reasoning_effort` on `task`. Effort is plugin agent frontmatter `effort` out of the box, or `~/.grok/roles/<role-key>.toml` when setup wrote a level. Independent verify is `independent-verifier` plus the model rule above.

You own every subagent's work. Review the diff and write your own summary, don't pass through what it said. Resume a finished child with `resume_from` on `task`, same `subagent_type`. A second opinion is the same prompt against a different `model`. Agreement is high-signal.
