...
- **Skill order.** pstack first, then user, then bundled and builtin. Table: `HARNESS.md` **Skill order**. Examples: `/tdd` before `/test-driven-development`, `/interrogate` before `/review`, `playbooks/babysit.md` before `/pr-babysit`, `pstack:how-explorer` before builtin `explore`. Skip a later column when pstack already covers the job.
