...
- **Skill order.** pstack first, then user, then bundled and builtin. Table: `HARNESS.md` **Skill order**. Examples: `/tdd` before `/test-driven-development` (only if `/tdd` is not loaded), `/interrogate` before `/review`, `playbooks/babysit.md` with no `/pr-babysit` fallback, `pstack:how-explorer` before builtin `explore`. Skip a later column when pstack already covers the job. Plugin vs native keep two inspect rows, with `collidesWith`. Invoke `/pstack:<name>` and open the plugin or git body. Never load `~/.grok/skills/<name>` as the pstack body. Do not copy plugin skills into `~/.grok/skills`.
...
Spawn with the **current host** primitive. Grok Build: this TUI's `spawn_subagent` (rust alias `task`; fields in `HARNESS.md`). Codex: `spawn_agent` / `wait_agent` per `references/codex-tools.md`. Claude Code: `Agent`. Do not invent Cursor `Task` fields (`readonly`, `environment`, `capability_mode`, `reasoning_effort`).

**Depth.** Grok Build sets `MAX_SUBAGENT_DEPTH` to 1. Only this parent session may call `spawn_subagent`. A child that calls it fails. Fan-out (`how`, `swarm`, `arena`, `interrogate`, independent verify) stays in the parent. A code-writing role child owns the diff directly. It does not spawn nested workers.

**Types.** Routed workflow skills set `subagent_type` to `pstack:<role-key>` (`pstack:feature`, `pstack:how-explainer`, `pstack:independent-verifier`, …) per [`../setup-pstack/references/resolve-effort.md`](../setup-pstack/references/resolve-effort.md). Ad-hoc helpers with no role key: `pstack:poteto-agent`. `/no-comments` uses `pstack:comment-sicko`. Independent verify: toml `independent-verifier` when that key is a detected slug different from the writer; no toml: `grok-4.6`; inherit-parent/`auto`/missing key: omit `model`.

**Defaults for every `spawn_subagent` call.** Set `background` when the parent must keep working (this TUI defaults `background` to false). File pointers, not inlined dumps. Resolve `model` from `~/.grok/pstack-models.toml` per [`../setup-pstack/references/resolve-model.md`](../setup-pstack/references/resolve-model.md). Absent file: send `grok-4.6` (omit if rejected). Missing key, `inherit-parent`, or `auto`: omit `model`. Isolation is `none` unless the child must not touch the writer's tree, then `worktree`. Join with `get_command_or_subagent_output` (`task_ids`, positive `timeout_ms`). Do not send `reasoning_effort` or `capability_mode` on spawn. Effort is plugin agent frontmatter `effort` out of the box, or `~/.grok/roles/pstack:<role-key>.toml` when setup wrote a level. Independent verify is `pstack:independent-verifier` plus the model rule above.

You own every subagent's work. Review the diff and write your own summary, don't pass through what it said. Resume a finished child with `resume_from` on `spawn_subagent`, same `subagent_type`. A second opinion is the same prompt against a different `model`. Agreement is high-signal.
...
Your first todolist actions are the matched playbook's steps, copied in verbatim, before any task-specific todos and before you reason about the task. The failure mode is reading a playbook then writing a bespoke plan that drops its named steps (`architect`, the throughput checkpoint). A step you choose not to do stays in the list with a one-line `skip: <reason>`; skipping silently is not allowed. Match the task to a playbook below, open git-tracked `skills/poteto-mode/playbooks/<name>.md`, and copy its steps in verbatim. Do not open `~/.grok/installed-plugins/pstack-*/skills/poteto-mode/playbooks/`. A comma-separated list of existing playbooks (investigation, shipping) is sequential matching. It is not Multi-phase-plan. That playbook writes a plan file and stops.
