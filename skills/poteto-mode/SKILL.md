...
- **Native first.** If a Grok Build builtin, bundled skill, or user skill covers the same job and is live in `grok inspect` skills (or is a pager slash / builtin agent), use it. pstack is the fallback. Table: `HARNESS.md` **Native first**. Examples: `/test-driven-development` before `/tdd`, `/review` before `/interrogate` for a single-reviewer pass, `/pr-babysit` for a GitHub PR, `/create-skill` for SKILL.md, `/loop` for overnight, builtin `explore` for a read-only spawn that does not need a pstack role overlay.
