...
through the host's canonical task and agent state. Gas City adapters MUST use
Gas City formulas and Beads for routing, retries, persistence, and fanout/fanin.
...
MUST expose the same version, skill paths, and agent path. The shared skill
paths MUST include `./skills/` and `./automations/benny-grok/skills/`. Neither
manifest may register plugin-global hooks.
