...
- [ ] 3.1 After the writers are on `origin/main`, bump `plugin.json` to `0.14.5-grokbuild.4` and land that commit.
- [ ] 3.2 From a host shell run `./scripts/release.sh`. Confirm `gh run list --workflow=release.yml` is non-empty and the GitHub Release for `v0.14.5-grokbuild.4` exists.
...
- [ ] 4.1 `openspec validate pstack-release-actions-proof --type change --strict` then archive after implementation is on `main`.
