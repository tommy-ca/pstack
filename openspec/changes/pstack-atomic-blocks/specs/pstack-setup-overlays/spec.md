...
Official `/setup-pstack` writes Cursor `~/.cursor/rules`. This port's `/setup-pstack` MUST write `~/.grok/pstack-models.toml` (`[models]`, `[effort]`) and `~/.grok/roles/pstack:<key>.toml` (`description`, `reasoning_effort`). It MUST NOT write Cursor mdc. Absent overlay: shipped `grok-4.6` plus agent frontmatter effort.
