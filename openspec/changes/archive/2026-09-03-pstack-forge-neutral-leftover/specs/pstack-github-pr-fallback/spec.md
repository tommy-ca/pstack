...
#### Scenario: orchestrate does not require Graphite

- **GIVEN** `skills/poteto-mode/playbooks/orchestrate.md`
- **WHEN** the forge-neutral verification test reads it
- **THEN** the playbook does not tell the stacker to run `gt`
- **AND** frontier recomputation uses `origin pr` or `gh pr`
- **AND** a regression that requires `gt` in that file fails the test
