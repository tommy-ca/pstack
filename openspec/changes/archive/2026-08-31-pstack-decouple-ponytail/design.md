...
2. **Invert the test.** Replace `test_essential_entries_include_user_ponytail` with a recouple lock: live setup and HARNESS contain no `/ponytail`. The main spec forbids naming it. Keep `plugin.json` free of ponytail. Do not scan `openspec/changes/archive/`.
