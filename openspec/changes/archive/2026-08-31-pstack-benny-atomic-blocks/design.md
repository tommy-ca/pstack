...
In-force ADRs: 0003 (plugin-installed live path, no plugin hooks) and 0004 (live path is plugin skills). ADR-0001 and ADR-0002 are superseded.

## Goals / Non-Goals

**Goals:**
- Specify Benny as atomic blocks (pack, triage, repro, safety, grok remap).
- Remap onto grok natives without plugin-global hooks. Live coordinators are `/benny-triage` and `/benny-repro`.
...
- Registering Cursor `setup-benny` / `triage-issue-reports` as slash skills.
...
2. **Hook only merge and force-push.** `fail-closed.sh` denies `gh pr merge`, `git push --force` / `-f`, and `gt merge`. Slack freeze and worker isolation stay prompt-enforced in `automations/benny-grok/skills`. Alternative was a PreToolUse matcher on Slack MCP names. Rejected. We do not know the live `server__tool` id, and fail-open on crash would still post.
...
- [Prompt-only Slack policy] -> Agents can still post a root message if they ignore `/benny-triage`. Mitigation is coordinator-only writes in that skill, plus no Slack credentials on children. Not a hook.
