...
`grok inspect --json` proves enable and trust. It is not leftover scanner. Plugin-agents uses inspect to require `pstack:swarm-workers`. `grok plugin validate` alone is not leftover scanner. `pytest` and `tests/test_verify_harness.py` are not leftover scanner.
