...
3. After enable, `grok inspect --json` `.agents[].name` includes `pstack:swarm-workers` and `pstack:how-explorer`, not the bare `how-explorer`. Type `/poteto-mode`. It does not auto-enter. `/setup-pstack` is optional. If inspect lists only `pstack:poteto-agent` and `pstack:comment-sicko`, Grok enabled the Claude marketplace copy. From a host shell run `grok --sandbox off plugin install /path/to/this/checkout --trust` then `grok --sandbox off plugin enable pstack`, and start a new session.
