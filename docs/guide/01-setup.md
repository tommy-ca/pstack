...
3. After enable, `grok inspect --json` `.agents[].name` includes `pstack:swarm-workers` and `pstack:how-explorer`, not the bare `how-explorer`. Type `/poteto-mode`. It does not auto-enter. `/setup-pstack` is optional. If inspect lacks `pstack:swarm-workers`, Grok is not loading this checkout. From a host shell run `grok --sandbox off plugin install tommy-ca/pstack --trust` then `grok --sandbox off plugin enable pstack`, and start a new session.
