...
The [Opening a PR playbook](../../skills/poteto-mode/playbooks/opening-a-pr.md) runs `/no-comments` on the diff and applies [`/unslop`](../../skills/unslop/SKILL.md) to the PR description and commit bodies. Remove narrating comments, unsupported guards, dead compatibility paths, and unrelated edits.
...
The division of labor is worth keeping straight. `/unslop` cleans slop out of prose, and `/no-comments` hands the comments to a reviewer who didn't write them.

**Pitfall:** cleanup is not optional polish. A diff with narrating comments and defensive dead weight reads as unfinished to reviewers, and the extra code is where the next bug hides. If the diff feels padded, run `/no-comments` before you commit, not after review calls it out.
