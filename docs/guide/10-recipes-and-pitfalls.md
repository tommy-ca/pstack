...
- **Treating `auto` as a model slug.** `auto` and `inherit-parent` mean omit `task.model` so the child inherits the parent. A missing toml or missing key is the same omit. [Setup](./01-setup.md) covers the roles.
