...
| Marketplace add / plugin enable | `grok plugin marketplace add` is catalog-only. It does not install pstack. Install with `grok --sandbox off plugin install tommy-ca/pstack --trust`. `grok plugin enable pstack` rewrites `~/.grok/config.toml`. If the sandbox returns `EROFS`, run that one host-shell command with `grok --sandbox off`. After enable, `grok inspect --json` `.agents[].name` must include `pstack:swarm-workers`. Inspect `plugins[].path` and `skills[]` `collidesWith` name the install tree and skill overlays. Missing `pstack:swarm-workers` means Grok is not loading this checkout (`tommy-ca/pstack`). Keep release/tag mechanics in `docs/guide/13-grok-natives.md`. | `09-plugins.md`; `14-headless-mode.md` |
