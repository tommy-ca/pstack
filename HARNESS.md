...
| Graphite `gt` / `graphite-base` | Optional if `gt` is on PATH. If `command -v gt` fails, rewrite every Graphite call site with [`skills/poteto-mode/references/github-pr-fallback.md`](./skills/poteto-mode/references/github-pr-fallback.md) (`gh pr create`, `gh pr view`, `gh pr checks`, `gh pr merge`). Playbook steps stay; the CLI is not assumed. Never `gh pr merge --auto` unless the PR targets protected trunk. | `github-pr-fallback.md` |
