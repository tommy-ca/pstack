...
| Herdr grok SessionStart | `herdr integration install grok` writes `~/.grok/hooks/herdr.json` and `herdr-agent-state.sh`. `grok --sandbox devbox` is herdr-tracked for install because `devbox` skips Direct global hook write protection. `homelab` cannot whitelist that dir with `read_write`. The pin stays. SessionStart tracking writes `$TMPDIR` and `$HERDR_SOCKET_PATH`, not the hooks dir. Install/update from a host shell, `grok --sandbox herdr-install`, `off`, or `devbox`. | `18-sandbox.md`; herdr `HERDR_INTEGRATION_ID=grok` |
