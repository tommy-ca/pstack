...
| Marketplace add / plugin enable | `grok plugin enable pstack` rewrites `~/.grok/config.toml`; if the sandbox returns `EROFS`, run that one host-shell command with `grok --sandbox off`. After enable, `grok inspect --json` `.agents[].name` must include `pstack:swarm-workers`. Missing that name means Grok is not loading this checkout (`tommy-ca/pstack`). Install with `grok --sandbox off plugin install tommy-ca/pstack --trust`. Keep release/tag mechanics in `docs/guide/13-grok-natives.md`. | `09-plugins.md`; `14-headless-mode.md` |
